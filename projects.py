...
def calc_variance():
  hours = [sum([hour[1] for hour in works.values()]) for works in workdays.values()]
  return pvariance(hours)
  ...
  for day in sorted(iter(workdays)):
    ...
    for asmt,event in sorted(work.items(), key=(lambda z:z[0])):
      if event[1]>0.02:
        ...
        if len(m)==1:
          m='0'+m
        key = hash(asmtArr[0]+'_'+asmtArr[1]+'_'+str(event[0]))
        if key not in workedSoFar:
          workedSoFar[key] = 0.0
        workedSoFar[key] += event[1]
        p = round(100.0*workedSoFar[key]/events[asmtArr[0]][asmtArr[1]][event[0]][1])
        insort(strs,(p,event[1],"  "+str(int(event[1]))+':'+m+"\t"+'('+str(p)+'% done)'+'\t'+asmtArr[0]+" "+asmtArr[1]+" "+events[asmtArr[0]][asmtArr[1]][event[0]][2]))
...
for line in lines:
  ...
  for d in range((eventday-startDay).days):
    day = startDay+ relativedelta.relativedelta(days=d)
    if day.toordinal() not in original_workdays:
      original_workdays[day.toordinal()] = {}
    original_workdays[day.toordinal()][line[3]+"_"+line[4]] = [
        eventI,
       float(line[0]) / float((eventday-startDay).days)]
...
while attempts<maxAttempts:
  ...
  startTime = now()
  variance_loss=inf
  while its<100 and now()-startTime<5 and variance_loss>10**(-6):
